...
class ExtendedGRPOTrainer(GRPOTrainer):
        ...
        if self.logging_completions["save_completions"]:
            ...
            def _reward_func_wrapper(reward_func):
                ...
                def reward_func_wrapper(*args, **kwargs):
                    ...
                    assert (
                        reward_key not in kwargs
                    ), f"[ExtendedGRPOTrainer.__init__] reward_key ({reward_key}) should not be in kwargs"
    ...
    def save_good_completions(self):
        ...
        assert (
            "_completion_id" not in logged_completions_buffer
        ), f"[ExtendedGRPOTrainer.log_good_completions] _completion_id should not already be in logged_completions_buffer"
        assert (
            "_global_step" not in logged_completions_buffer
        ), f"[ExtendedGRPOTrainer.log_good_completions] _global_step should not already be in logged_completions_buffer"
        assert (
            "_rewards" not in logged_completions_buffer
        ), f"[ExtendedGRPOTrainer.log_good_completions] _rewards should not already be in logged_completions_buffer"
        assert (
            "_min_reward_threshold" not in logged_completions_buffer
        ), f"[ExtendedGRPOTrainer.log_good_completions] _min_reward_threshold should not already be in logged_completions_buffer"
        ...
        if self.logging_completions["min_reward_seen"] is None:
            ...
            self.logging_completions["min_reward_seen"] = min(
        ...
        if self.logging_completions["max_reward_seen"] is None:
            ...
            self.logging_completions["max_reward_seen"] = max(
        ...
        json.dump(
            metadata, open(f"{self.logging_completions['save_completions_dir']}/metadata.json", mode="w"), indent=4
        )
        ...
        json.dump(
            good_completions,
            open(
                f"{self.logging_completions['save_completions_dir']}/good_completions_chunk_{self.logging_completions['current_chunk_id']}.json",
                mode="w",
            ),
        )
