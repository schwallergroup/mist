...
class SmilesHydrogen(RLTask):
    ...
    def __init__(self, **kwargs):
        ...
        smiles_hydrogen_prompt_guiding_path = os.path.join(
            os.path.dirname(__file__), "smiles_hydrogen_prompt_guiding.json"
        )
    ...
    def accuracy_reward(
        ...
        if self.log_custom_metrics:
            ...
            self.custom_metrics["addH/accuracy_reward"].extend(
    ...
    def smiles_validity_reward(self, completions, question_category, **kwargs):
        ...
        # Logging custom metrics
        if self.log_custom_metrics:
            self.custom_metrics["addH/smiles_validity_reward"].extend(
                [r for r, c in zip(rewards, question_category) if c == "addH"]
            )
    ...
    def tanimoto_accuracy_reward(
        ...
        # Logging custom metrics
        if self.log_custom_metrics:
            self.custom_metrics["addH/tanimoto_accuracy_reward"].extend(
                [r for r, c in zip(rewards, question_category) if c == "addH"]
            )
    ...
    def levenstein_accuracy_reward(
        ...
        # Logging custom metrics
        if self.log_custom_metrics:
            self.custom_metrics["addH/levenstein_accuracy_reward"].extend(
                [r for r, c in zip(rewards, question_category) if c == "addH"]
            )
