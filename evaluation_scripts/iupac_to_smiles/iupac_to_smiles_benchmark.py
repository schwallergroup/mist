...
def generate_prompts(data, is_reasoning: bool, mist_system_prompt: bool = True):
    if mist_system_prompt:
        if is_reasoning:
            return [
                "<|im_start|>assistant\You are a useful chemistry assistant and answer the SMILES generation based question below. Think your answer in steps in terms of molecule substituent postion and SMILES structures inside the <think>...</think> tags and then give your final answer inside <answer>...</answer> tags.<|im_end|>\n<|im_start|>user\\"
                + item[0]["input"].replace(" <answer>", "")
            ...
            return [
                "<|im_start|>assistant\You are a useful chemistry assistant and answer the SMILES generation based question below. Just give your answer alone inside the <answer>...</answer> tags.<|im_end|>\n<|im_start|>user\\"
                + item[0]["input"].replace("<answer>", "")
                + "<|im_end|>\n<|im_start|>assistant\Your response:\n<answer>"
                for item in data
            ]
        ...
        return [
            template.format(
                item[0]["input"].replace("<answer>", "").replace("[START_SMILES] ", "").replace(" [END_MOL]", "")
            )
            for item in data
        ]
...
def _evaluate(
    prompts: list[str], test_data, llm, sampling_params, outpath: str, reasoning=True, mist_system_prompt=True
):
...
def main(args):
    ...
    _evaluate(
        ...
        llm,
        sampling_params,
        os.path.join(args.out_dir, "direct_output.txt"),
        reasoning=False,
        mist_system_prompt=is_mist_system_prompt,
    )
    _evaluate(
        reasoning_prompts,
        data,
        llm,
        sampling_params,
        os.path.join(args.out_dir, "reasoning_output.txt"),
        reasoning=True,
        mist_system_prompt=is_mist_system_prompt,
    )
