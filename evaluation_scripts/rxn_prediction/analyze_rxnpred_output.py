...
def arg_parse():
    ...
    parser.add_argument(
        "--per_sample_eval_csv",
        type=str,
        default="per_sample_results.csv",
        help="Path to the per sample evaluation CSV file.",
    )
    parser.add_argument("--task_mode", type=str, required=True)
    parser.add_argument(
        "--result_file", type=str, default="eval_results.txt", help="Path to the evaluation results JSON file."
    )
    parser.add_argument(
        "--datapath", type=str, default="/data/share/USPTO_480k_clean_no_sft/", help="Path to the evaluation data."
    )
...
def extract_smiles(text, prompt="", relax=False):
    ...
    if (
        res is None and relax
    ):  # if relax is True, extract the longest SMILES in the reponse that is different from those in the prompt
...
def extract_info_response(response: str):
    ...
    return {
        ...
        "extract_smiles_relax": extracted_smiles_relax,
    }
...
def eval(response: str):
    ...
    return {
        "example_id": response_info["example_num"],
...
def summary_eval(per_response_eval_csv: str):
    ...
    return {
        "accuracy": sum(per_example_correct.values()) / len(per_example_correct),
